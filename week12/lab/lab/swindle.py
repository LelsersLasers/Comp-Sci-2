...
def readBookDatabase(filename: str) -> list[Book]:
    """
    ...
    Return Val: a list of Book obects
    """
    ...
    with open(filename, "r") as infile:
        for line in infile:
            line = line.strip()
            items = line.split(",")
            # items = [title, author, year, filename]
...
class Swindle(object):
    ...
    def __init__(self, owner: str):
        ...
        self.available_books = readBookDatabase("bookdb.txt")  # list of Book objects
        self.owner: str = owner
        self.owned_books: list[Book] = []
    ...
    def show_owned(self) -> None:
        """prints the books the user owns"""
        ...
        if num_owned == 0:
            ...
            for idx in range(num_owned):
                print("%2i: %s" % (idx + 1, self.owned_books[idx].to_string()))
        return

    def show_available(self) -> None:
        """prints the books the user can buy"""
        ...
        if num_available == 0:
            ...
            for idx in range(num_available):
                print("%2i: %s" % (idx + 1, self.available_books[idx].to_string()))
        return
    ...
    def buy(self) -> None:
        """adds user chosen book to owned books"""
        self.show_available()
